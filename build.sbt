...
val alpakkaKinesis = "com.lightbend.akka" %% "akka-stream-alpakka-kinesis" % "3.0.3"
...
lazy val main = (project in file("main")).settings(libraryDependencies ++= Seq(akkaActor, slf4j)).dependsOn(event, http)

lazy val `kinesis-demo` =
  (project in file(".")).aggregate(event, http)
